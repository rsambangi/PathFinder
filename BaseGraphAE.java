 ...
 */
public class BaseGraphAE<NodeType, EdgeType extends Number> {
  ...
  protected class Edge implements Comparable<Edge> {
    ...
    /**
     * This method allows the comparison between edge object based on their weight.
     * 
     * @param o, the edge to compare with
     * @return 1 is this.edge is bigger, -1 if this.edge is smaller than o, and 0 if they are the
     *         same
     */
    @Override
    public int compareTo(BaseGraphAE<NodeType, EdgeType>.Edge o) {
      if (data.doubleValue() > o.data.doubleValue()) {
        return 1;
      }
      if (data.doubleValue() < o.data.doubleValue()) {
        return -1;
    ...
    /**
     * ToString method to format the connected edges of the graph.
     * 
     * @return "Edge Weight: " + data, the formatted edges
     */
    @Override
    public String toString() {
      return "Edge Weight: " + data;
    }
  }
   ...
   * pred and succ already exists, update the data stored in hat edge to be weight.
   * 
  ...
  /**
   * Check if edge is in the graph. * @param pred the data item contained in the source node for the
   * edge
   * 
   * @param succ the data item contained in the target node for the edge
  ...
  protected Edge getEdgeHelper(NodeType pred, NodeType succ) {
    ...
    throw new NoSuchElementException("No edge from " + pred.toString() + " to " + succ.toString());
  }
